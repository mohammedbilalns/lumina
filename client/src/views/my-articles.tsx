    ...
    <div className="min-h-screen bg-[#FBFBFA] text-[#111111] font-sans selection:bg-[#f8cb5b]/30 pb-20">
      ...
      <main className="container mx-auto px-6 py-12 max-w-4xl">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-10">
          ...
          <Link
            to="/article/create"
            className="px-5 py-2.5 bg-[#f8cb5b] hover:bg-[#f2c94c] text-[#0b2226] rounded-md font-medium transition-colors flex items-center justify-center gap-2 shrink-0"
          >
        ...
        <div className="space-y-4">
          {MY_ARTICLES.map((article) => (
            <div
              key={article.id}
              className="relative bg-white border border-[#EAEAEA] rounded-xl p-6 hover:shadow-sm transition-all flex flex-col md:flex-row md:items-center justify-between gap-6"
            >
                ...
                <div className="flex items-center gap-3 mb-2">
                  <h2 className="text-xl font-serif font-medium text-[#0b2226] line-clamp-1">
                    <Link
                      ...
                      className="hover:text-[#13383d] transition-colors"
                    >
                      {article.title}
                    </Link>
                  </h2>
                  <span
                    className={`px-2.5 py-0.5 text-xs font-semibold uppercase tracking-wider rounded-md border ${
              ...
              <div className="flex items-center gap-2 shrink-0 border-t md:border-t-0 border-[#EAEAEA] pt-4 md:pt-0 relative z-10">
                <Link
                  to="/article/$id/edit"
                  params={{ id: article.id.toString() }}
                  className="flex items-center gap-2 px-4 py-2 bg-white border border-[#EAEAEA] hover:bg-slate-50 text-slate-600 rounded-md transition-colors font-medium text-sm"
                >
